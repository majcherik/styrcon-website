            ...
            {(state) => (
                <div
                    className={cx(
                        ...
                        !state.isDisabled && "group-hover:bg-gray-100",
                ...
                >
                    {Icon && (
                        <Icon
                            aria-hidden="true"
                            className={cx("mr-2 size-4 shrink-0 stroke-[2.25px]", state.isDisabled ? "text-gray-400" : "text-gray-600")}
                        />
                    )}
                    ...
                    <span
                        className={cx(
                            ...
                            state.isDisabled ? "text-gray-400" : "text-gray-900",
                    ...
                    {addon && (
                        <span
                            className={cx(
                                "ml-3 shrink-0 rounded px-1 py-px text-xs font-medium ring-1 ring-gray-300 ring-inset",
                                state.isDisabled ? "text-gray-400" : "text-gray-600",
                            )}
                        >
                            {addon}
                        </span>
...
const DropdownPopover = (props: DropdownPopoverProps) => {
    return (
        <AriaPopover
            ...
            className={(state) =>
                cx(
                    "w-62 origin-(--trigger-anchor-point) overflow-auto rounded-lg bg-white shadow-lg ring-1 ring-gray-200 will-change-transform z-[60]",
                    state.isEntering &&
...
const DropdownDotsButton = (props: AriaButtonProps & RefAttributes<HTMLButtonElement>) => {
    return (
        <AriaButton
            ...
            className={(state) =>
                cx(
                    "cursor-pointer rounded-md text-gray-600 outline-focus-ring transition duration-100 ease-linear",
                    (state.isPressed || state.isHovered) && "text-gray-700",
